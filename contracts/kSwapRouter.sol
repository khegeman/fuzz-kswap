...
contract kSwapRouter is IkSwapSwapCallback,IkSwapMintCallback {
    ...
    function addLiquidity(address _pool, uint256 _amountADesired,
        uint256 _amountBDesired,
        uint256 _amountAMin,
        uint256 _amountBMin) external  returns (uint256 amount0_, uint256 amount1_, uint256 liquidity_) {
        pool = _pool;

         (amount0_, amount1_) = _addLiquidity(
            _pool,
            _amountADesired,
            _amountBDesired,
            _amountAMin,
